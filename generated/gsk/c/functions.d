...
__gshared extern(C)
{
	...
	int gsk_render_node_write_to_file(GskRenderNode* node, char* filename, GError** err);
	...
	GskTransform* gsk_transform_skew(GskTransform* next, float skewX, float skewY);
	void gsk_transform_to_2d(GskTransform* self, float* outXx, float* outYx, float* outXy, float* outYy, float* outDx, float* outDy);
	void gsk_transform_to_2d_components(GskTransform* self, float* outSkewX, float* outSkewY, float* outScaleX, float* outScaleY, float* outAngle, float* outDx, float* outDy);
