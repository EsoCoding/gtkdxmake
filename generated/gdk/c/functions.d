...
__gshared extern(C)
{
	...
	GdkContentFormats* gdk_content_formats_parse(const(char)* string_);
	...
	GdkGLContext* gdk_display_create_gl_context(GdkDisplay* self, GError** err);
	...
	int gdk_display_prepare_gl(GdkDisplay* self, GError** err);
	...
	GdkFileList* gdk_file_list_new_from_array(GFile** files, size_t nFiles);
	GdkFileList* gdk_file_list_new_from_list(GSList* files);
	GSList* gdk_file_list_get_files(GdkFileList* fileList);
	...
	int gdk_gl_context_is_shared(GdkGLContext* self, GdkGLContext* other);
	...
	GdkTexture* gdk_texture_new_from_bytes(GBytes* bytes, GError** err);
	GdkTexture* gdk_texture_new_from_file(GFile* file, GError** err);
	GdkTexture* gdk_texture_new_from_filename(char* path, GError** err);
	...
	int gdk_texture_save_to_png(GdkTexture* texture, char* filename);
	GBytes* gdk_texture_save_to_png_bytes(GdkTexture* texture);
	int gdk_texture_save_to_tiff(GdkTexture* texture, char* filename);
	...
	int gdk_toplevel_titlebar_gesture(GdkToplevel* toplevel, GdkTitlebarGesture gesture);
